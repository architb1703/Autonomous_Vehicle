...
def find_max(boxes_final, labels_final):
    x1 = x2 = y1= y2 = -1
    area = 0
    for i in range(len(labels_final)):
        if(labels_final[i]!='car'):
            continue
        temp = (boxes_final[i].xmax-boxes_final[i].xmin)*(boxes_final[i].ymax-boxes_final[i].ymin)
        if(temp>area):
            x1,x2,y1,y2 = boxes_final[i].xmin, boxes_final[i].xmax, boxes_final[i].ymin , boxes_final[i].ymax
            area = temp
    return([x1,x2,y1,y2])

#Function to implement complete pipeline of image file
def object_detection_yolov3(imageFolderPath):
    os.mkdir(imageFolderPath+'/'+'filtered')
    for img in tqdm(os.listdir(imageFolderPath)):
      savePath = imageFolderPath+'/'+'filtered/'+img
      img = imageFolderPath+'/'+img
      try:
      ...
      (x1,x2,y1,y2) = find_max(boxes_final, labels_final)
      if(x1==-1):
          continue
      else:
          try:
              img = cv2.imread(img)
              print(savePath, x1,x2)
              cv2.imwrite(savePath, img[y1:y2, x1:x2, :])
          except:
              pass      
# input("Press any key to continue")

object_detection_yolov3('./dataset/silver')
